...
with open("./combine/K_COVID19.csv", 'r') as file:
    ...
    for i,line in enumerate(file_read):
        ...
        # checking duplicate region_id & checking region_id == "NULL"
        ...
        print(sql_data)
        query = """INSERT INTO `regionInfo`(region_code, province, city, latitude, longitude, elementary_school_count, kindergarten_count,university_count, academy_ratio,elderly_population_ratio,elderly_alone_ratio,nursing_home_count) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"""
        sql_data = tuple(sql_data)
        #print(sql_data)
        #for debug
        try:
            cursor.execute(query, sql_data)
            print("[OK] Inserting [%s] to patientInfo"%(line[col_list['region_code']]))
